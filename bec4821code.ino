...
String url = "http://hirutech.000webhostapp.com/addvalue.php";  //URL for HTTP-POST-REQUEST
// analog I/O pins
...
//if(!name1.startsWith(",")){
  mySerial.println("AT+HTTPPARA=URL,"+url+"?devid=A1&locdata="+String(lati,6)+"*"+String(lon,6)
  +"&dt="+gps.date.year()+"-"+gps.date.month()+"-"+gps.date.day()+" "+gps.time.hour()+":"+gps.time.minute()
  +":"+gps.time.second()+"&alti="+gps.altitude.meters()+"&tempi="+tempf+"&humi="+humidity+"&lightl="+light_lvl
  +"&pressr="+pressure+"&satsv="+gps.satellites.value()+"&satdir="+gps.course.deg()+"&spd="+gps.speed.kmph());
